...
define jboss::instance_12::config (
  ...
  # Link to first wildfly-12 instance, in order to have available jbosscli on a predefined path
  exec { "/opt/wildfly-12-${instance_name}":
    command => "ln -s ${jboss_inst_folder} /opt/wildfly-12",
    user    => root,
    group   => root,
    unless  => 'test -e /opt/wildfly-12',
  }
  ...
  # Custom jboss-cli.sh that set JAVA_HOME consistently with WildFly-12
